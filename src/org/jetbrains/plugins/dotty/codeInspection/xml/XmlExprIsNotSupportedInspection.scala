...
class XmlExprIsNotSupportedInspection extends AbstractInspection(id, name) {
  override def actionFor(holder: ProblemsHolder): PartialFunction[PsiElement, Any] = {
    case typeElement: ScXmlExprImpl =>
      holder.registerProblem(typeElement, message, ERROR, new ReplaceXmlExprQuickFix(typeElement))
  }
}

class ReplaceXmlExprQuickFix(token: PsiElement) extends AbstractFixOnPsiElement(name, token) {

  def replaceXmlExpr(psiElem: ScalaPsiElement, level: Int): Unit = {
    for (e <- psiElem.getChildren) {
      e match {
        case sc: ScalaPsiElement =>
          replaceXmlExpr(sc, level + 2)
        case _ =>
      }
    }
    psiElem match {
