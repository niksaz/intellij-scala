...
class ModuleExtDataServiceTest extends ProjectDataServiceTestCase with UsefulTestCaseHelper {
  ...
  def testModuleIsNull(): Unit = {
    val testProject = new project {
      ...
      arbitraryNodes += new ModuleExtNode(new ModuleExtData(Some(Version("2.11.5")), Seq.empty, Seq.empty, None, Seq.empty))
    }.build.toDataNode
  ...
  def testScalaSdkForEvictedVersion(): Unit = {
    ...
    val projectData = new project {
      ...
      modules += new javaModule {
        ...
        arbitraryNodes += new ModuleExtNode(new ModuleExtData(Some(Version(evictedVersion)), Seq.empty, Seq.empty, None, Seq.empty))
      }
    }.build.toDataNode
  ...
  private def generateProject(scalaVersion: Option[String], scalaLibraryVersion: Option[String], scalacOptions: Seq[String], jdk: Option[Sdk], javacOptions: Seq[String]): DataNode[ProjectData] =
    new project {
      ...
      modules += new javaModule {
        ...
        arbitraryNodes += new ModuleExtNode(new ModuleExtData(scalaVersion.map(Version(_)), Seq.empty, scalacOptions, jdk, javacOptions))
      }
    }.build.toDataNode
